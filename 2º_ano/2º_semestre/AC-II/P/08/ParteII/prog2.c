...
int main(void){
	// Configuraçao T1 tipo A 2 Hz e T3 tipo B 100Hz com interrupções
	// T1:
	...
	TRISD = (TRISD & 0xFF9F);
	TRISB = (TRISB & 0x80FF);
	TRISB = (TRISB | 0x0009); // switch 1 e 3 entrada 
...
void _int_(VECTOR_TIMER1) isr_T1(void){
	...
	counter = upDown ? (counter + 1) % 30 : ((counter - 1 + 30) % 30);
