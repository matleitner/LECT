...
struct _SortedList {
  ...
  struct _ListNode* head;     // inicio the head of the List
  struct _ListNode* tail;     // fim the tail of the List
  struct _ListNode* current;  // atual the current node
...
void* ListRemoveCurrent(List* l) {
  ...
  if (l->currentPos == 0)
	...
	struct _ListNode *curr = l->current;	
 	struct _ListNode *node = l->head;
