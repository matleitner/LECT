...
def printTabela(tabela):
    print()
    print("{:19s} {:>3} {:>3} {:>3} {:>3} {:>3}:{:<3} {:>3} {:>3}".format(

            "Equipa", "J", "V", "E", "D", "GM", "GS", "P","D"))

    for reg in tabela:
        nome,v,e,d,gm,gs = reg
        print("{:19s} {:3d} {:3d} {:3d} {:3d} {:3d}:{:<3d} {:3d} {:3d}".format(
                nome, numJogos(reg), v, e, d, gm, gs, pontos(reg), diferenca(reg)))
...
def main():
    ...
    tab = sorted(tabela, key = diferenca, reverse = True)
    printTabela(tab)
    ...
    tab = sorted(tabela,key = lambda reg: [pontos(reg), diferenca(reg)] , reverse=True )
