 ...
 *  
 *  \pre text != NULL
 *  \pre ch != NULL && num_chars_utf8(ch) == 1
 *
 ...
 *  
 *  \pre text != NULL
 *  \pre res == NULL || (max_length > 0 && length_vargs_string_list(text) <= max_length)
 *
 *  \return the concatenated string
 */
...
/**
 * \brief determine the number of digits the decimal representation of the number has
 * \param [in] num The number to process
